      ...
      <header className="sticky top-0 z-20 border-b border-ink-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex min-h-14 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
          ...
          <nav className="flex items-center gap-1 overflow-x-auto" aria-label="Application navigation">
            <NavLink to="/app" end className={linkClass}>Studio</NavLink>
            <NavLink to="/app/generate" className={linkClass}>Create</NavLink>
            <NavLink to="/app/advisor" className={linkClass}>Advisor</NavLink>
            <NavLink to="/app/profile" className={linkClass}>Profile</NavLink>
            {isAdmin && <NavLink to="/admin" className={({ isActive }) => `rounded-lg px-3 py-1.5 text-sm font-medium transition-colors ${isActive ? "bg-ink-900 text-white" : "text-ink-600 hover:bg-ink-100 hover:text-ink-900"}`}>Admin</NavLink>}
            <div className="ml-2 flex shrink-0 items-center gap-2.5 border-l border-ink-200 pl-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-brand-100 text-sm font-semibold text-brand-700">{initials}</div>
              <button onClick={onSignOut} className="btn-ghost px-2.5 py-1.5 text-xs">Sign out</button>
            </div>
          </nav>
        </div>
      </header>
