...
function Protected({ children }: { children: React.ReactNode }) {
  ...
  if (loading) return <div className="flex min-h-screen items-center justify-center"><div className="h-8 w-8 animate-spin rounded-full border-2 border-ink-200 border-t-brand-600" /></div>;
  return session ? <>{children}</> : <Navigate to="/signin" replace />;
}

function AdminHome() {
  return <div className="card p-6"><h1 className="text-xl font-semibold text-ink-900">Strike Command Center</h1><p className="mt-2 text-sm text-ink-500">Admin workspace is protected and ready for the operational dashboard modules.</p></div>;
}
...
export function App() {
  return <BrowserRouter><AuthProvider><AdminProvider><AppRoutes /></AdminProvider></AuthProvider></BrowserRouter>;
}
