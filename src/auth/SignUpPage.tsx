...
export function SignUpPage() {
  ...
  const onSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setFormError(null);
    const ok = validateAll([name, emailF, password]);
    if (!ok) return;
    ...
    // Create the profile row immediately (RLS: insert_own_profile, auth.uid matches)
    if (data.user) {
    ...
    // If email confirmation is OFF (default in this project), a session is returned.
    if (data.session) {
